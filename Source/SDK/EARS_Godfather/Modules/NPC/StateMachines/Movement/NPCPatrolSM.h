...
{
	class NPCPatrolSM : public EARS::Framework::AnimateStateMachine
	{
	public:

		NPCPatrolSM() = delete;
		NPCPatrolSM(uint32_t TableID, EARS::StateMachineSys::StateMachineParams* SMParams);
		virtual ~NPCPatrolSM();

		//~ Begin AnimateStateMachine Interface
		virtual uint32_t GetStateMachineID() const override { return 0x7A086EC7; }
		...
		static EARS::StateMachineSys::StateMachine* S_NPCPatrolSM_FactoryFn(unsigned int InID, EARS::StateMachineSys::StateMachineParams* InSMParams);

	private:
		char m_Padding[0x34];
	};
	static_assert(sizeof(NPCPatrolSM) == 0x84);
}
