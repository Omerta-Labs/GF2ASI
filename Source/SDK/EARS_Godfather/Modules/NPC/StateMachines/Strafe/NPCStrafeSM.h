...
{
	class NPCStrafeSM : public EARS::Framework::AnimateStateMachine
	{
	public:

		NPCStrafeSM() = delete;
		NPCStrafeSM(uint32_t TableID, EARS::StateMachineSys::StateMachineParams* SMParams);
		virtual ~NPCStrafeSM();

		//~ Begin AnimateStateMachine Interface
		virtual uint32_t GetStateMachineID() const override { return 0x0F33262CA; }
		...
		static EARS::StateMachineSys::StateMachine* S_NPCStrafeSM_FactoryFn(unsigned int InID, EARS::StateMachineSys::StateMachineParams* InSMParams);

	private:
		char m_Padding[0x3C];
	};
	static_assert(sizeof(NPCStrafeSM) == 0x8C);
}
